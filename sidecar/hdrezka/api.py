...
class HdRezkaApi():
	...
	def getStream(self, season=None, episode=None, translation=None,
		...
		def getStreamMovie(self, translation_id):
			# VENDOR PATCH (ours): HDRezka's get_movie now requires the camrip/ads/director
			# flags that the page passes to sof.tv.initCDNMoviesEvents(...). Without them the
			# CDN responds success:true but url:false. Parse them from the page, default 0.
			flags = {"is_camrip": 0, "is_ads": 0, "is_director": 0}
			m = re.search(r'initCDNMoviesEvents\(\s*\d+\s*,\s*\d+\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)', self.page.text)
			if m:
				flags = {"is_camrip": int(m.group(1)), "is_ads": int(m.group(2)), "is_director": int(m.group(3))}
			return makeRequest({
				"id": self.id,
				"translator_id": translation_id,
				"action": "get_movie",
